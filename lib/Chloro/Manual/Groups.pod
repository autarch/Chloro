...
Here's a group definition for our phone number group:
    ...
    group phone_number => (
        repetition_field => 'phone_number_id',
        (
...
one of the values in the C<repetition_field>, something like "phone_number.42".
...
(our C<repetition_field>) will always contain an array reference.

It's also possible that a phone number id will not have any associated data
under the C<phone_number> key, if the group was empty.
