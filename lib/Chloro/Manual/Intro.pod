...
When you C<use Chloro>, your class is a form. A form class consumes the
L<Chloro::Role::Form> role. Since you're still using Moose, you can inherit
from other classes, define attributes, consumer other roles, etc.
...
message object available from C<< $error->message() >>. The message will be
either a L<Chloro::ErrorMessage::Invalid> or L<Chloro::ErrorMessage::Missing>
object.
...
are any. These can either be strings or objects that do the
L<Chloro::Role::ErrorMessage> role.
